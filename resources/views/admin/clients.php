...
<div class="p-10 text-center text-slate-500">No clients yet.</div>
<?php else: ?>
...
<td class="px-6 py-4 text-sm font-semibold text-[#0f172a]"><?= htmlspecialchars($c['full_name']) ?></td>
<td class="px-6 py-4 text-sm text-slate-600"><?= htmlspecialchars($c['email']) ?></td>
<td class="px-6 py-4"><span class="inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-100 text-slate-700"><?= htmlspecialchars($c['status']) ?></span></td>
<td class="px-6 py-4 text-right"><a href="<?= base_url('admin/clients/' . $c['id']) ?>" class="inline-flex items-center gap-1 text-sm font-semibold text-sky-600 hover:text-sky-700 transition-colors">View <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg></a></td>
</tr>
<?php endforeach; ?>
</tbody></table>
<?php endif; ?>
